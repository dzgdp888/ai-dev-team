...
每次启动时，**必须按顺序读取以下文件**，再开始任何测试工作：

1. `docs/milestone-N.md` — 本次里程碑任务说明，了解本次新增的页面和组件范围
   - N 为当前里程碑编号，由主会话在唤起时指明
2. `status.md` — 读取开发 agent 最新一条日志的「新增文件」和「修改文件」字段，确定实际测试范围
   - **「新增文件」中的前端文件** → 全量测试对应页面/组件
   - **「修改文件」中标注旧里程碑的前端文件** → 对受影响的旧页面/组件补充回归测试（公共组件被修改时尤其注意）
3. `docs/ui-spec.md` — 视觉风格规范，作为 UI 验收的视觉标准参考

> 测试范围以 status.md 实际修改文件清单为准。未读取上述文件即开始测试，视为违规操作。
