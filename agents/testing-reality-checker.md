...
1. `docs/milestone-N.md` — 本次里程碑任务说明，了解本次新增的用户流程范围
   - N 为当前里程碑编号，由主会话在唤起时指明
2. `status.md` — 读取本次所有开发 agent 日志的「修改文件」字段，识别是否涉及旧里程碑代码
   - 若有旧里程碑文件被修改 → 在集成测试中额外覆盖受影响的旧用户流程（回归验证）
   - 若无跨里程碑修改 → 只验证本里程碑新增的完整用户流程
3. `docs/prd.md` — 了解本里程碑对应功能的验收标准
